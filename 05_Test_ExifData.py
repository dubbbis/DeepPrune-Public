...
if len(uploaded_files) > 1:
    ...
    for i in range(len(images)):
        exif_data = extract_exif_data(uploaded_files[i].name)
        st.write(f"**Image {image_paths[i]}** - Latitude: {exif_data['Latitude']}, Longitude: {exif_data['Longitude']}, Altitude: {exif_data['Altitude']}")
