...
if uploaded_files:
    ...
    st.write("File names:", [f.name for f in uploaded_files])
    exif_results = []
    images = {}
    image_paths = []
    resized_images = {}  # resized image dictionary for performance optimization
    ...
    for uploaded_file in uploaded_files:
        ...
        # NUEVO: crear versión redimensionada desde el inicio
        image_rgb = image.convert("RGB")
        image_np = cv2.cvtColor(np.array(image_rgb), cv2.COLOR_RGB2BGR)
        resized_images[uploaded_file.name] = cv2.resize(image_np, (512, 512))
        st.write(f"Resized: {uploaded_file.name}")

    exif_df = pd.DataFrame(exif_results)
    st.write("### Debug: EXIF Data Extracted")
    st.write(exif_df.head())
    ...
    selected_group = st.sidebar.selectbox("Select a group for overlap analysis", [1, 2, 3])
    ...
    for idx, group in enumerate(image_groups):
        with st.expander(f"Group {idx + 1} ({len(group)} images)"):
            group_imgs = [images[img['Image']] for img in group if img['Image'] in images]
            st.image(group_imgs, caption=[img['Image'] for img in group], width=150)

    if transition_images:
        st.write("### Transition Images:")
        transition_imgs = [images[img['Image']] for img in transition_images if img['Image'] in images]
        st.image(transition_imgs, caption=[img['Image'] for img in transition_images], width=150)
    ...
    if analyze_button and len(images) > 2:
        ...
        for i in range(1, len(image_paths) - 1):
            ...
            img1 = resized_images[img1_path]
            img2 = resized_images[img2_path]
            img3 = resized_images[img3_path]
