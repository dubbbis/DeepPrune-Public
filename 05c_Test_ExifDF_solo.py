...
def extract_exif_data(image_file):
    ...
    """
    try:
        # Open the image file in binary mode (from Streamlit upload)
...
# Streamlit UI Title
st.title("Extract EXIF Data from Images")

# Upload multiple images through Streamlit file uploader
uploaded_files = st.file_uploader("Upload Images", type=["jpg", "jpeg", "png"], accept_multiple_files=True)

# Process images if at least one is uploaded
if uploaded_files:
    exif_results = []
    
    for uploaded_file in uploaded_files:
        exif_data = extract_exif_data(uploaded_file)  # Ahora pasamos el archivo directamente
...
# Checkbox to optionally perform overlap calculation
perform_calculation = st.sidebar.checkbox("Perform Overlap Calculation", value=False)

if perform_calculation and len(uploaded_files) > 1:
