...
def group_images_by_location(image_data):
    """Groups images into flight path columns based on latitude and longitude thresholds."""
    lat_threshold = 0.00005  # Latitude threshold for grouping within a column
    lon_threshold = 0.000005  # Longitude threshold to detect column transitions
    ...
    for i in range(1, len(image_data)):
        ...
        if lat_diff <= lat_threshold and lon_diff <= lon_threshold:
            current_group.append(curr)  # Add to current group if within thresholds
        else:
...
if uploaded_files:
    ...
    # Group images by location using the calibrated thresholds
    image_groups = group_images_by_location(image_data_list)
